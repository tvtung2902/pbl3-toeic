		...
		<div class="row">
			...
			<div class="col-12">
				<h3>Câu <%= q.getOrderNumber()%>. <%=q.getQuestionContent()%> </h3>
					    <input type="radio" id="contentAnswerA<%=q.getQuestionID()%>" name="answer<%=q.getQuestionID()%>" value="A">
	      				<label for="contentAnswerA<%=q.getQuestionID()%>"><%=q.getContentAnswerA()%></label><br>
	        			<input type="radio" id="contentAnswerB<%=q.getQuestionID()%>" name="answer<%=q.getQuestionID()%>" value="B">
	        			<label for="contentAnswerB<%=q.getQuestionID()%>"><%=q.getContentAnswerB()%></label><br>
	        			<input type="radio" id="contentAnswerC<%=q.getQuestionID()%>" name="answer<%=q.getQuestionID()%>" value="C">
	        			<label for="contentAnswerC<%=q.getQuestionID()%>"><%=q.getContentAnswerC()%></label><br>
						<input type="radio" id="contentAnswerD<%=q.getQuestionID()%>" name="answer<%=q.getQuestionID()%>" value="D">
